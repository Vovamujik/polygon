...
struct Graph {
    ...
    std::string createKey(double lon, double lat) {
        // Генерирует уникальный ключ для узла на основе его координат
    ...
    Node* findNearestNode(double lon, double lat) {
        ...
        // Проходит по всем узлам и ищет ближайший.
    ...
    void parseFromFile(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            char separator;
            double lon, lat;
            iss >> lon >> separator >> lat >> separator; // Считывем координаты узла
    ...
    double astar(Node* start, Node* end) {
        // Поиск кратчайшего пути методом A* с использованием эвристики
...
int main() {
    ...
    graph.parseFromFile("spb_graph-2.txt");

    double startLon = 30.33395, startLat = 60.066931; // 30.33395   60.066931
    double endLon = 30.308108, endLat = 59.957238; // 30.308108    59.957238
